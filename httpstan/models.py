...
def _prepare_build_services_extension_module(program_code: str, extra_compile_args: Optional[List[str]] = None):
...
async def build_services_extension_module(program_code: str, extra_compile_args: Optional[List[str]] = None) -> str:
    ...
    """
    extensions, build_lib = _prepare_build_services_extension_module(program_code, extra_compile_args)
...
def build_services_extension_module_sync(program_code: str, extra_compile_args: Optional[List[str]] = None) -> str:
    """Compile a model-specific stan::services extension module.
    ...
    """
    extensions, build_lib = _prepare_build_services_extension_module(program_code, extra_compile_args)
    compiler_output = httpstan.build_ext.run_build_ext(extensions, build_lib)
    return compiler_output
